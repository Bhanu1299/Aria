...
def test_returns_formatted_string_with_facts():
    ...
    with patch("aria.plugins.memory.vector_store.ChromaStore.get", return_value=_fake_store(facts)):
        from aria.plugins.memory import context_injector
...
def test_returns_empty_string_when_no_facts():
    with patch("aria.plugins.memory.vector_store.ChromaStore.get", return_value=_fake_store([])):
        from aria.plugins.memory import context_injector
...
def test_returns_empty_string_on_error():
    with patch("aria.plugins.memory.vector_store.ChromaStore.get", side_effect=Exception("boom")):
        from aria.plugins.memory import context_injector
        result = context_injector.build("anything")
    assert result == ""
