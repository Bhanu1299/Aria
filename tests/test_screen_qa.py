...
@pytest.mark.parametrize("command", [
    ...
    "explain me what is highlighted",
    "what is highlighted",
...
@pytest.mark.parametrize("command", [
    ...
    "point out the error on my screen",
    "circle the thing I should click",
])
def test_screen_explain_precheck_skips_classifier(command):
    with patch.object(router, "_classify", side_effect=AssertionError("classifier should not be called")):
        intent = router.route(command)
    assert intent["type"] == "screen_explain"
    assert intent["query"] == command.strip()
